...
describe("talk overview buttons (#293)", () => {
    ...
    it("„Für alle Raids“ opens one modal with one character select and the status for every raid", async () => {
        ...
        expect(modal.title).toBe("Für alle 7 Raids anmelden");
        ...
        expect(modal.components.map((c) => [c.type, c.label, c.component.custom_id])).toEqual([
            [18, "Charaktere · Specs für alle Raids", "all"],
            [18, "Status für alle", "status"],
        ]);
        const select = modal.components[0].component;
        expect(select).toMatchObject({ type: 3, min_values: 1, max_values: 2 });
        expect(select.options.map((o) => [o.label, o.default])).toEqual([["Zibbo · Heilig", true], ["Zibbowar · Schutz", false]]);
        expect(modal.components[0].description).toContain("Oberster gewählter = 1. Wahl");
    });
    ...
    it("without characters or raids answers only the member, with where to go", async () => {
        ...
        expect(none.reply.mock.calls[0][0]).toMatchObject({ flags: MessageFlags.Ephemeral, content: expect.stringContaining("Charaktere mit Spec") });
        ...
        expect(empty.reply.mock.calls[0][0].content).toContain("keine Raids");
    });
    ...
    it("saves „alle“ per raid: first pick = choice, class and deadline refusals named per raid", async () => {
        ...
        expect(lines[0]).toMatch(/^✅ \*\*Raid 1\*\* · \S+ \d\d\.\d\d\.: Zibbo · Heilig, \+Zibbowar · Schutz$/);
        expect(lines[1]).toMatch(/^⛔ \*\*Raid 2\*\*.*Keiner der gewählten Charaktere passt \(Zibbo · Heilig übersprungen: Klasse passt nicht zu diesem Raid; /);
        expect(lines[2]).toMatch(/^⛔ \*\*Raid 3\*\*.*Anmeldeschluss/);
...
describe("Mehrere Raids wählen … (#293)", () => {
    ...
    it("shows step 1 only to the member: every raid preselected, status, Weiter", async () => {
        ...
        expect(payload.embeds[0].title).toBe("Für welche Raids?");
        ...
        expect(statusRow.components[0].options.map((o) => o.label)).toEqual(["Dabei", "Vielleicht", "Spät", "Bank", "Abmelden"]);
        expect(goRow.components[0]).toMatchObject({ custom_id: `signup-multi:${token}:go:0`, label: "Weiter: Charaktere (Raids 1–5)" });
    });
    ...
    it("keeps the picked raids and status, then pages the modal in fives with Weiter", async () => {
        ...
        expect(partial.embeds[0].title).toBe("Bisher 2 von 5 Raids gespeichert");
        expect(partial.embeds[0].description).toContain("⏭️ **Raid 3**");
        expect(partial.embeds[0].description).toMatch(/✅ \*\*Raid 1\*\*.*Zibbo · Heilig – Vielleicht/);
        expect(partial.components[0].components[0]).toMatchObject({ custom_id: `signup-multi:${token}:go:1`, label: "Weiter: Raid 6" });
        ...
        expect(second.title).toBe("Mit welchen Charakteren? (2/2)");
        ...
        expect(done.embeds[0].title).toBe("Anmeldung: 3 von 6 Raids gespeichert");
        expect(done.components[0].components.map((b) => b.label)).toEqual(["Meine Anmeldungen"]);
    ...
    it("refuses an expired or foreign token without saving anything", async () => {
        ...
        expect(foreign.update.mock.calls[0][0].content).toContain("abgelaufen");
        ...
        expect(expired.reply.mock.calls[0][0]).toMatchObject({ flags: MessageFlags.Ephemeral, content: expect.stringContaining("abgelaufen") });
...
describe("Mehrere Charaktere … an einem Event (#293)", () => {
    it("opens the modal for that one raid with the current characters preselected in their order", async () => {
        ...
        const modal = i.showModal.mock.calls[0][0];
        expect(modal.title).toBe("Mit welchen Charakteren?");
        const select = modal.components[0].component;
    ...
    it("refuses after the deadline for „Dabei“ and without the raider role", async () => {
        ...
        expect(late.reply.mock.calls[0][0].content).toContain("Anmeldeschluss");
        ...
        expect(noRole.reply.mock.calls[0][0].content).toBe("Für diesen Raid brauchst du eine Raider-Rolle.");
