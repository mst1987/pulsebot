// Gear loaded from one Warcraft-Logs report on request (src/stores/logGearStore.js).
...
jest.mock("../../src/stores/logStore", () => ({ listLogs: (...a) => mockListLogs(...a) }));

const fs = require("fs");
const {
    loadLogGear, gearFromReport, getLogGear, listLogGear, clearLogGear, recentLogs, MAX_LOGS,
} = require("../../src/stores/logGearStore");
...
describe("stores/logGearStore — one report, one raider", () => {
...
describe("stores/logGearStore — loading and keeping", () => {
    ...
    it("tolerates a corrupt file", () => {
        const { LOG_GEAR_FILE } = require("../../src/stores/logGearStore");
        fs.__store.set(LOG_GEAR_FILE, "{not json");
        expect(listLogGear()).toEqual([]);
    });
});

describe("stores/logGearStore — the logs offered to pick from", () => {
