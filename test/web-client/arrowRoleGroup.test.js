...
const assignEn = loadTs("lib/assign.ts", { t: makeT("en"), ...mention, ...cr });
const am = loadTs("lib/assignModal.ts", { ...assign, ...cr });
...
describe("role references in rows and steps", () => {
    ...
    it("resolve to the role group (icon, name), never to players, never open", () => {
        ...
        expect(assignEn.resolveAssignee("role:healer", ctx).label).toBe("Healers");
