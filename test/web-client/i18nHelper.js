...
function loadTs(rel, inject = {}) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        out.push(line.replace(rel === "lib/assign.ts" ? /^\} as Record<.*>;$/ : /(?!)/, "};").replace(rel === "lib/setupEditor.ts" ? /^(\s*const \w+): Partial<.*?> = / : /(?!)/, "$1 = ").replace(/^export (default )?/, "").replace(rel === "lib/raidplan.ts" || rel === "lib/assign.ts" ? / as [A-Za-z_]\w*(\["\w+"\])?/g : /(?!)/g, ""));
    }
    const js = out.join("\n");
