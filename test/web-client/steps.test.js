...
// the "du" form, the @ mentions, the resolution of a step, the library, the starters; plus the structure of the components and the texts.
const fs = require("fs");
const path = require("path");
const { loadTs, makeT } = require("./i18nHelper");

const cr = loadTs("lib/classRefs.ts");
const st = loadTs("lib/steps.ts", { t: makeT("de"), ...cr });
const stEn = loadTs("lib/steps.ts", { t: makeT("en"), ...cr });
...
describe("timing, sentence, du form", () => {
    it("names the timing", () => {
        ...
        expect(stEn.timingLabel({ kind: "interval", from: 20, to: null, text: "" })).toBe("every 20 s");
    });
