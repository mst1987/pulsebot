...
describe("web/report/render — Buffs topic of a fight", () => {
    it("adds a Buffs topic listing who lacked what: missing, run out, wrong role", () => {
        ...
        expect(html).toMatch(/Buffs<span class="n(?: mid| bad)?">2 · 2 fehlten<\/span>/);
...
describe("web/report/render — Raid-Buffs section", () => {
...
describe("web/report/render — Buffs on the player page", () => {
    ...
    it("tells a buff set after the pull apart from one that ran out", () => {
        ...
        expect(player).toMatch(/Buffs<span class="n(?: mid| bad)?">1 · 1 fehlten<\/span>/);
...
describe("web/report/render — buffs read off the events", () => {
    ...
    it("names the inferred buffs and the open cells on a fight's Buffs topic and draws an open cell as a neutral ribbon row", () => {
        const { renderPlayerPage } = require("../../../src/web/report/render.js");
        const tl = timeline();
        const f = tl.fights[0];
...
describe("web/report/render — buffs the log cannot show", () => {
