...
- **Palette (left):** the eight raid marks, the slots (tank, healer, dps, group, label), zones (four types, rectangle or ellipse) and shapes (arrow, line, text). **Drag an entry onto the board or click it** (Pointer Events; an entry that never moved is a click and lands near the middle). Not built yet: class/spec icons, the "Encounter" tab and the icon search (follow-up).
