...
| der React-Client `src/web-client/src/**` | Vitest, neben dem Modul: `lib/raidplan/index.ts` → `lib/raidplan.test.ts` (oder `raidplan.<thema>.test.ts`), `pages/ChannelsPage.tsx` → `pages/ChannelsPage.test.tsx`; strukturelle Konventionen unter `test/web-client/conventions/` (siehe „Web-Client“) |
