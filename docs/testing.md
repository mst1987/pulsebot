...
| eine Suite, die für eine Datei zu groß ist, oder ein Thema quer zu einem Modul | `<modul>.<thema>.test.js` daneben: `test/web/loot/lootCouncil.gear.test.js`; im Client `src/web-client/src/lib/raidplan/raidplan.slots.test.ts` |
| der React-Client `src/web-client/src/**` | Vitest, neben dem Modul: `lib/tableSort.ts` → `lib/tableSort.test.ts`, die Board-Logik `lib/raidplan/` als `lib/raidplan/raidplan.<thema>.test.ts`, `pages/ChannelsPage.tsx` → `pages/ChannelsPage.test.tsx`; strukturelle Konventionen unter `test/web-client/conventions/` (siehe „Web-Client“) |
...
- **Wo:** neben dem Modul als `*.test.ts` bzw. `*.test.tsx` (`lib/raidplan/assign.ts` → `lib/raidplan/assign.test.ts`, `pages/profile/ProfilePage.tsx` → `pages/profile/ProfilePage.test.tsx`; ein Thema quer dazu als `ClaPage.actions.test.tsx`). Imports explizit aus `"vitest"`, keine Globals.
...
Was kein Render-Test sehen kann, prüft weiter der Root-Jest am Quelltext: jedes Modul mit eigenem Stylesheet und eigenem Klassen-Präfix (`cssNamespaces`), lazy geladene Routen (`codeSplitting`), die API-Aufteilung (`apiSplit`), kein durchgereichter CSRF-Token (`csrf`), keine deutschen Literale in irgendeiner Quelldatei des Clients und Datum/Zeitzone nur über `lib/format.ts` (`i18n`, dazu `i18n-*`), kein nativer `title`, keine zweite Kopie eines Bausteins (`icons`, `raidLoader`, `useDismiss`, `listSection`, `resultToasts`, …). Dateien lesen sie über `test/web-client/clientSource.js` (`read`, `sourceFiles`, `clientSources`, `pageSources`, `readWorkspace`, `stripComments`, `dictionary`); jeder Durchlauf über den Client lässt dessen eigene Tests aus. `read()` nimmt auch einen Ordner und liefert dann alle seine Quellen als eine (eine in Teile zerlegte Seite wird als Ganzes geprüft, #438), `pageSources()` sind alle Seiten und Komponenten samt der Ordner in `SPLIT_FOLDERS`, `readWorkspace()` die Raidplan-Arbeitsfläche mit ihren Hooks. Eine neue Konvention kommt dorthin — **Verhalten** einer Komponente dagegen gehört als Render-Test in den Client, nicht als `toContain` auf den Quelltext.
