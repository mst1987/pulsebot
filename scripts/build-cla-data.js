// Generates src/config/generated/claData.json (read by src/config/claData.js) from the cloned Apps Script source + extracted CSVs.
// Run once (and again if the CLA source changes). Not used at runtime.
//
// Inputs: scripts/data-sources/cla/*.csv (checked in) and the Apps Script source
// in scripts/data-sources/cla/appsscript/, which is git-ignored - fetch it first
// with `clasp clone <scriptId>` into that folder (scripts/data-sources/cla/README.md).
const fs = require("fs");
const path = require("path");
const { writeGeneratedJson } = require("./lib/generatedJson");

const REF = path.join(__dirname, "data-sources", "cla");
...
// --- emit -----------------------------------------------------------------
// The key meanings are documented in src/config/claData.js.
const data = {
    ENCHANTABLE_SLOTS: [0, 2, 4, 6, 7, 8, 9, 14, 15, 16],
    GEM_QUALITY: { ignore: 0, common: 1, uncommon: 2, rare: 3, epic: 4 },
    META_GEM_IDS: META,
    YELLOW_GEM_IDS: YELLOW,
    RED_GEM_IDS: RED,
    BLUE_GEM_IDS: BLUE,
    UNCUT_GEM_IDS: UNCUT,
    ACCEPTABLE_UNCOMMON_GEM_IDS: ACCEPTABLE_UNCOMMON,
    ACCEPTABLE_RARE_GEM_IDS: ACCEPTABLE_RARE,
...
const outPath = writeGeneratedJson("claData.json", data);
