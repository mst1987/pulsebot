...
- **The need score** (`needScore()`) is 40 % drought + 30 % loot share + 30 % BiS gap, and its three parts go out with it so the page can show the reasoning. It is deliberately simple and visible — a council can argue with a number whose parts it can see.
