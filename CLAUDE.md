...
- **The raid-category filter narrows the *roster*, not just the loot.** Filtering only the items left every other raid's casters standing there with "0 Items" and a maximum drought — on top of the very ranking the page exists for. `categoryMembers()` decides who belongs: the maintained raider→character assignment (`raiderCharactersStore`, Einstellungen → Kategorien) unioned with everyone who won loot there. The assignment is the half that knows a raider who has **never won anything in that raid**, which is exactly the raider a council looks for — so a category without one falls back to the loot and the payload says so (`filter.categorySource`). A category nothing is known about is not filtered at all, since an empty list would read as "this raid has no casters".
