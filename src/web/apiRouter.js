...
const { getSession, postActiveGuild, postLang, getViewAs, postViewAs } = require("./apiRoutes/session");
...
const {
    ...
    postPingText: postSetupPingText, postSearchMessage: postSetupSearch, postSearchText: postSetupSearchText,
...
async function route(pathname, req, res, url) {
    ...
    if (pathname === "/api/session/view-as" && req.method === "GET") {
        getViewAs(req, res);
        return true;
    }
    if (pathname === "/api/session/view-as" && req.method === "POST") {
        await postViewAs(req, res);
        return true;
    }
    ...
    if (pathname === "/api/raids/setup/search/text" && req.method === "POST") {
        await postSetupSearchText(req, res);
        return true;
    }
