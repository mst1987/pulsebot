...
// creature, every one checked against the icon CDN; docs/raidplan.md). `similar` marks them as placeholders.
let MOB_ICONS = {};
try { MOB_ICONS = require("../config/mobIcons.json").icons || {}; } catch { /* not generated yet: the mobs have no icon */ }

const mob = (slug, name, kind, instanceId, bossKey = "") => ({
    id: `d:${slug}`, name, kind, instanceId, bossKey, icon: (MOB_ICONS[`d:${slug}`] || {}).icon || "", similar: !!MOB_ICONS[`d:${slug}`], note: "",
});
