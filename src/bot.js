...
const {
    ...
    getAuctionMessage,
    getItemsToShow,
} = require("./functions/responses.js");
const {
    getAllSignUps,
    getCategorySetups,
    getSetupsFromEvents,
} = require("./functions/raidhelper.js");
const {
    getTargetMessage,
    updateHighestBids,
} = require("./functions/legendary.js");
const {
    toTimestamp,
...
const { showBidModal, showConfirmationModal } = require("./functions/auction.js");
...
client.on("interactionCreate", async(interaction) => {
            ...
            if (commandName === "gdkpraids") {
                ...
                let missingSignUps = await raidhelper.getMissingSignUps(
                    interaction.user.id
                );

                if (channelsInCategory) {
                    // Filter Signups for GDKP category
                    const noSignUps = missingSignUps.filter((signUpChannelId) =>
                        channelsInCategory.includes(signUpChannelId)
                    );
                    const formattedMissingSignUps = noSignUps
                        .map((channelId) => `<#${channelId}>`)
                        .join(`\n`);

                    // Filter Signups for GDKP category
                    const GDKPSignUps = signUpChannelIDs.filter((signUpChannelId) =>
                        channelsInCategory.includes(signUpChannelId.channelId)
                    );
                    const SignUpsWithSpecs = GDKPSignUps.map((dataObject) => {
                        const matchingSignUps = dataObject.signUps.filter(
                    ...
                    const formattedGDKPSignUps = SignUpsWithSpecs.map(
                        (channelId) => `<#${channelId.channelId}>\n ${channelId.specs}\n`
                    ).join(`\n`);
                    await botReply(
                        interaction,
                        messages.gdkpraids.successTitle,
                        messages.gdkpraids.missingSignups.replace(
                            "___replace___",
                            formattedMissingSignUps
                        ) +
                        messages.gdkpraids.signups.replace(
                            "___replace___",
            ...
            if (commandName === "bid-custom") {
                ...
                if (interaction.isModalSubmit()) {
                    if (interaction.customId === 'bidModal') {
                        ...
                        if (isNaN(bid)) {
                            await interaction.reply({ content: 'Bitte geben Sie eine gültige Zahl ein.', ephemeral: true });
                            return;
                        }
                        ...
                        const confirmation = interaction.fields.getTextInputValue('confirmBid').toLowerCase();
                        const bid = pendingBids.get(interaction.user.id);

                        if (confirmation === 'ja') {
                            // Hier implementieren Sie die Logik zum Platzieren des Gebots
                            await interaction.reply({ content: `Ihr Gebot von ${bid} Gold wurde erfolgreich platziert!`, ephemeral: true });
                        } else {
                            await interaction.reply({ content: 'Gebot abgebrochen.', ephemeral: true });
                        }
