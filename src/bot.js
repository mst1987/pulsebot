...
client.on('interactionCreate', async(interaction) => {
    ...
    if (commandName === 'endauction') {
        ...
        if (response.type === 'success') {
            botReply(interaction, 'Auktion beendet!', `Die Auktion wurde beendet!\n\nHöchstbietender und damit Gewimmer von ${findServerEmoji(interaction, 'shadowmourne')} ${response.legendary.name} für den Raid **${response.legendary.raid}** ist ${response.winner}!\n\n${findServerEmoji(interaction, 'peepoParty')} Gratulation und viel Spaß damit! ${findServerEmoji(interaction, 'peepoParty')}`);
        } else {
            botReply(interaction, 'Fehler', 'Ein Fehler ist vorgefallen...');
        }
    }
