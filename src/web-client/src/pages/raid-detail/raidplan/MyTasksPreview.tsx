...
export default function MyTasksPreview({ rows, board, players, catalog, me }: { rows: RaidplanAssignment[]; board: RaidplanBoard; players: Map<string, RaidplanPlayer>; catalog: Catalog | null; me: string[] }) {
    const t = useT();
    const ctx = useMemo(() => ({ slots: board.slots, players, catalog, groupColors: board.groupColors, groupMarks: board.groupMarks }), [board.slots, board.groupColors, board.groupMarks, players, catalog]);
