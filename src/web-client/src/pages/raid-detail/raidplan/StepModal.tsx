                ...
                <section className={`rp-stm-sec${cur(2)}`} aria-labelledby="rp-stm-l2">
                    ...
                    <div className="rp-stm-grid" role="tabpanel">
                        ...
                        {tab === "groups" && groups.map((g) => {
                            const key = `group:${g}`;
                            return <button key={key} type="button" className={`rp-amb-tile rp-st-pick${has(key) ? " is-on" : ""}`} aria-pressed={has(key)} onClick={() => toggle(key)}><span className="rp-gdot" aria-hidden="true" style={{ "--rp-gdot": groupColor(board.groupColors, g) } as React.CSSProperties} /><span className="rp-amb-name">{t("raidBoard.slot.group", { n: g })}</span>{has(key) && <Check size={13} className="rp-amb-check" aria-hidden="true" />}</button>;
                        })}
