...
export function ActionIcon({ action, size = 30, label }: { action: string; size?: number; label?: string }) {
    ...
    return (
        <span className={`rp-acticon is-${ACTION_GROUP[action] || "flow"}`} style={{ "--rp-ai": `${size}px` } as CSSProperties} role={label ? "img" : undefined} aria-label={label} aria-hidden={label ? undefined : true}>
