...
describe("resolving references", () => {
    ...
    it("a raider, a group, a mark and text", async () => {
        ...
        expect(await inLang("en", () => lib.resolveTarget({ kind: "group", ref: "2" }, ctx).label)).toBe("Group 2");
    });
});
...
describe("wiring and texts", () => {
    it("has every type and picker text in both languages", () => {
        for (const file of [deBoard, enBoard]) {
            const d = file.assign;
